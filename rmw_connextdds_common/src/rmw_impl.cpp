    ...
    )
{
    ...
    RMW_CONNEXT_LOG_DEBUG_A("endpoint resource history: "
        "kind=%d, "
        "depth=%d",
        history->kind,
        history->depth);
    ...
    std::string *const type_name)
{
    ...
    if (RMW_RET_OK !=
            ctx->assert_topic(
                ...
                topic_created))
    {
        RMW_CONNEXT_LOG_ERROR_A("failed to assert topic: "
            "name=%s, "
            "type=%s",
            fqtopic_name.c_str(),
            type_support->type_name())
...
RMW_Connext_Publisher::finalize()
{
    ...
    if (this->created_topic)
    {
        ...
        RMW_CONNEXT_LOG_DEBUG_A("deleting topic: name=%s",
...
RMW_Connext_WaitSet::detach()
{
    for (auto && subscriber : this->attached_subscribers)
    {
        ...
        RMW_CONNEXT_LOG_DEBUG_A("[wait] detached subscriber: "
            "waitset=%p, "
            "condition=%p, "
            "sub=%p",
            (void*)this->waitset,
    ...
    for (auto && service : this->attached_services)
    {
        ...
        RMW_CONNEXT_LOG_DEBUG_A("[wait] detached service: "
            "waitset=%p, "
            "condition=%p, "
            "service=%p",
            (void*)this->waitset,
    ...
    for (auto && client : this->attached_clients)
    {
        ...
        RMW_CONNEXT_LOG_DEBUG_A("[wait] detached client: "
            "waitset=%p, "
            "condition=%p, "
            "client=%p",
            (void*)this->waitset,
    ...
    rmw_events_t * evs)
{
    ...
    if (refresh_attach)
    {
        RMW_CONNEXT_LOG_DEBUG_A("[wait] refresh attach: "
            "subs=%d (%lu), "
            "gcs=%d (%lu), "
            "srvs=%d (%lu), "
            "cls=%d (%lu), "
        ...
        if (nullptr != subs && subs->subscriber_count > 0)
        {
            ...
            for (size_t i = 0; i < subs->subscriber_count; i++)
            {
                ...
                RMW_CONNEXT_LOG_DEBUG_A("[wait] attached subscriber: "
                    "waitset=%p, "
                    "condition=%p, "
                    "sub=%p",
                    (void*)this->waitset,
        ...
        if (nullptr != gcs && gcs->guard_condition_count > 0)
        {
            ...
            for (size_t i = 0; i < gcs->guard_condition_count; i++)
            {
                ...
                RMW_CONNEXT_LOG_DEBUG_A("[wait] attached guard condition: "
                    "waitset=%p, "
                    "condition=%p",
                    (void*)this->waitset,
                    (void*)el)
        ...
        if (nullptr != srvs && srvs->service_count > 0)
        {
            ...
            for (size_t i = 0; i < srvs->service_count; i++)
            {
                ...
                RMW_CONNEXT_LOG_DEBUG_A("[wait] attached service: "
                    "waitset=%p, "
                    "condition=%p, "
                    "service=%p",
                    (void*)this->waitset,
        ...
        if (nullptr != cls && cls->client_count > 0)
        {
            ...
            for (size_t i = 0; i < cls->client_count; i++)
            {
                ...
                RMW_CONNEXT_LOG_DEBUG_A("[wait] attached client: "
                    "waitset=%p, "
                    "condition=%p, "
                    "client=%p",
                    (void*)this->waitset,
...
{\
    ...
    {
        RMW_CONNEXT_LOG_ERROR_A("unexpected active conditions: "
            "waitset=%p, "
            "triggered=%lu, "
            "active=%lu",
            (void*)this->waitset,
    ...
    const rmw_qos_profile_t *const qos_policies)
{
    ...
    RMW_CONNEXT_LOG_DEBUG_A("creating reply subscriber: "
        "service=%s, "
        "topic=%s",
        svc_name,
        reply_topic.c_str())
